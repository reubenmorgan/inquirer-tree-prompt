...
class TreePrompt extends BasePrompt {
  ...
  _installKeyHandlers() {
    ...
    const validation = this.handleSubmitEvents(
      events.line.pipe(
        map(() =>
          this.valueFor(this.opt.multiple ? this.selectedList[0] : this.active)
        )
      )
    );
  ...
  async addValidity(node) {
    if (typeof node.isValid === "undefined") {
      if (this.opt.validate) {
        node.isValid = await this.opt.validate(
          this.valueFor(node),
          this.answers
        );
      } else {
  ...
  render(error) {
    ...
    if (this.status === "answered") {
      let answer;
      if (this.opt.multiple) {
        answer = this.selectedList
          .map((item) => this.shortFor(item, true))
          .join(", ");
      } else {
  ...
  createTreeContent(node = this.tree, indent = 2) {
    ...
    children.forEach((child) => {
      ...
      if (this.opt.multiple) {
        prefix += this.selectedList.includes(child)
          ? figures.radioOn
          : figures.radioOff;
        prefix += " ";
      }
      ...
      const showValue =
        " ".repeat(indent) + prefix + this.nameFor(child, isFinal) + "\n";
  ...
  shortFor(node, isFinal = false) {
    return typeof node.short !== "undefined"
      ? node.short
      : this.nameFor(node, isFinal);
  }
  ...
  onSubmit(state) {
    ...
    this.done(
      this.opt.multiple
        ? this.selectedList.map((item) => this.valueFor(item))
        : state.value
    );
  }
